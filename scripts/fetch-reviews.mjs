 ...
 * fetch-reviews.mjs
 * ─────────────────
 * Fetches all Google reviews for Buzz Off Natural Pest Prevention via SerpAPI
 ...
 *
 * Re-run any time you want fresh reviews. .github/workflows/review-refresh-monthly.yml
 * runs this on the 1st of each month and commits the result, so the site's
 * rating and count track the live listing instead of freezing at fetch time.
 *
 * NOTE on sort_by: "ratingHigh" silently truncates. On this listing it returned
 * 24 of 40 reviews and stopped paginating. "newestFirst" returns the full set,
 * which is also the order the site wants.
 */
...
async function getDataId() {
  const params = new URLSearchParams({
    engine: "google_maps",
    q: "Buzz Off Natural Pest Prevention Rancho Cucamonga CA",
    api_key: API_KEY,
  });
