...
import yelpData from "../../../data/yelp-reviews.json";
...
// Live profile totals - these drive the stats bar, the "selection of X of Y"
// line, and the profile links. Google refreshes from the API (see
// .github/scripts/phase2/fetch-gbp-reviews.mjs); Yelp is maintained by hand
// because Yelp has no public review API.
const REVIEW_COUNTS = {
  google: googleData.totalReviewCount,
  yelp: yelpData.totalReviewCount,
};

const TOTAL_REVIEWS = REVIEW_COUNTS.google + REVIEW_COUNTS.yelp;
...
/** Alternate Google and Yelp cards so the grid never runs as one long block of a single source. */
...
const REVIEWS: Review[] = interleave(
  googleData.reviews.map((r) => ({ ...r, source: "google" as const })),
  yelpData.reviews.map((r) => ({ ...r, source: "yelp" as const })),
);
...
function YelpGlyph({ size = 12 }: { size?: number }) {
  return (
...
function SourceBadge({ source }: { source: "google" | "yelp" }) {
  return (
    ...
    >
      {source === "google" ? <GoogleGlyph /> : <YelpGlyph />}
...
function ProfileLink({ source }: { source: "google" | "yelp" }) {
  ...
  const count = isGoogle ? REVIEW_COUNTS.google : REVIEW_COUNTS.yelp;
  return (
    <a
    ...
    >
      <span style={{ color: isGoogle ? "#4285F4" : "#D32323", display: "inline-flex" }}>
        {isGoogle ? <GoogleGlyph size={16} /> : <YelpGlyph size={16} />}
      </span>
      Read all {count} on {isGoogle ? "Google" : "Yelp"}
      ...
      <main>
          ...
          <div className="max-w-[900px] mx-auto px-[clamp(20px,4vw,48px)]">
            <FadeIn>
              <div className="grid grid-cols-3 gap-6 text-center">
                ...
                <div>
                  <p className="font-display text-[clamp(2rem,4vw,3rem)] leading-none" style={{ color: "#1A5C32" }}>{TOTAL_REVIEWS}</p>
        ...
        <section className="py-[clamp(48px,6vw,80px)]" style={{ background: "#EDEADE" }}>
          <div className="max-w-[1200px] mx-auto px-[clamp(20px,4vw,48px)]">
            <FadeIn>
              <p className="font-body text-[0.72rem] font-bold tracking-[0.2em] uppercase text-center mb-8" style={{ color: "#1C2B1E99" }}>
                {REVIEWS.length < TOTAL_REVIEWS
                  ? `A selection of ${REVIEWS.length} of our ${TOTAL_REVIEWS} reviews`
                  : `All ${TOTAL_REVIEWS} reviews`}
              </p>
            </FadeIn>
